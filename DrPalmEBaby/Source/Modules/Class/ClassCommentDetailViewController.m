//
//  ClassCommentDetailViewController.m
//  DrPalm4Ebaby2
//
...
@interface ClassCommentDetailViewController () <ClassRequestOperatorDelegate, EGORefreshTableHeaderDelegate, RequestImageViewDelegate> {
    BOOL _reloading;
...
@implementation ClassCommentDetailViewController
...
- (void)setupNavigationBar {
    ...
    titleLabel.text = NSLocalizedString(@"ClassCommentDetailNavigationTitle", nil);
...
- (void)reloadData:(BOOL)isReloadView {
    ...
    self.itemArray = [ClassDataManager anwserContentListWithEventId:self.classAnwserMan.anwserMan_id eventId:self.item.event_id];
    if(isReloadView) {
...
- (BOOL)loadFromServer {
    ...
    return [self.requestOperator getAnswerContent:self.item.event_id aswpubid:self.classAnwserMan.anwserMan_id lasttime:0];
}
