//
//  ClassVideoListViewController.m
//  DrPalm4EBaby
//
...
@interface ClassVideoListViewController () <ClassRequestOperatorDelegate, EGORefreshTableHeaderDelegate>{
    BOOL _reloading;
...
@implementation ClassVideoListViewController
...
- (void)operateFinish:(id)data requestType:(ClassRequestOperatorStatus)type {
    ...
    switch(type){
        case ClassRequestOperator_GetEventList: {
            [self reloadData:YES];
            [LastUpdateDataManager updateLastUpdateWithCategory:Category_ClassVide lastupdate:[NSDate date] hasUser:YES];
        }break;
        default:break;
    }
}
