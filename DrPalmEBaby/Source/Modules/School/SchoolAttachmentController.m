//
//  SchoolAlbumnDetailViewController.m
//  DrPalm4EBaby
//
...
@interface SchoolAttachmentController () <RequestImageViewDelegate, SchoolAttachmentDownloaderDelegate> {
    NSInteger _curIndex;
...
@implementation SchoolAttachmentController
...
- (void)reloadData:(BOOL)isReloadView{
    ...
    self.itemArray = [SchoolDataManager imagesWithNewsId:self.item.story_id];
...
- (void)setupNavigationBar {
    ...
    titleLabel.text = NSLocalizedString(@"SchoolAttachmentDetailNavigationTitle", nil);
...
- (IBAction)downloadBatAction:(id)sender {
    ...
    if(self.itemArray.count > 0) {
        ...
        for(SchoolImage *schoolImage in self.itemArray) {
            SchoolFile *file = schoolImage.fullImage;
            if(file.data == nil) {
                downloadcount++;
...
- (void)saveToImageAlbum:(BOOL)batsave index:(NSInteger)index
{
    ...
    if(batsave) {
        ...
        SchoolImage *schoolImage = [self.itemArray objectAtIndex:index];
        SchoolFile *file = schoolImage.fullImage;
        if(file.data != nil) {
            saveCount++;
            [savearray addObject:file.data];
...
- (void)attachmentDownloader:(SchoolAttachmentDownloader*)attachmentDownloader downloadFinish:(NSData*)data contentType:(NSString*)contentType
{
    ...
    SchoolFile* file = [SchoolDataManager fileWithUrl:attachmentDownloader.url isLocal:NO];
    if(nil!= file)
    {
        file.data = data;
...
- (UIView *)viewForDetail:(DynamicContainView *)dynamicContainView {
    ...
    if(self.itemArray && self.itemArray.count > 0) {
        ...
        NSString *commentString = image.caption;
...
- (void)pagingScrollView:(PZPagingScrollView *)pagingScrollView preparePageViewForDisplay:(UIView *)pageView forIndex:(NSUInteger)index {
    ...
    requestView.delegate = self;
    requestView.imageUrl = schoolImage.fullImage.path;
    requestView.imageData = schoolImage.fullImage.data;
