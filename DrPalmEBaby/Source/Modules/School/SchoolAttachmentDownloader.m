...
@interface SchoolAttachmentDownloader()
@property (nonatomic, copy) NSString *contentType;
@property (nonatomic, retain) NSURLConnection *urlConnection;
@end

@implementation SchoolAttachmentDownloader
...
- (void)connection:(NSURLConnection *)connection didReceiveResponse:(NSURLResponse *)response 
{
	if ([response isKindOfClass:[NSHTTPURLResponse class]]){
        ...
        if ([self.delegate respondsToSelector:@selector(attachmentDownloader:startDownload:)]) {
            if (![self.delegate attachmentDownloader:self startDownload:self.contentType]) {
                [self stopDownload];
            }
        }
    }
