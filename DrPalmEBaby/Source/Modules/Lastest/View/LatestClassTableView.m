...
#import "LatestClassTableView.h"
#import "LatestTableViewCell.h"
#include "ClassDataManager.h"

@implementation LatestClassTableView
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    static NSString *cellIdentifier = @"LatestTableViewCell";
    if(indexPath.row < self.items.count) {
        ClassEventCategory *item = [self.items objectAtIndex:indexPath.row];
        ...
        NSInteger unReadCount = [item.unreadcount intValue];
        if(unReadCount > 0) {
            [cell.logoView setBadgeValue:[NSString stringWithFormat:@"%d", unReadCount]];
        }
        cell.titleLabel.text = NSLocalizedString(item.title, nil);
        
        cell.descLabel.text = item.titleofLastEvent;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if([self.tableViewDelegate respondsToSelector:@selector(tableView:didSelectLatestEvent:)]) {
        [self.tableViewDelegate tableView:self didSelectLatestEvent:[self.items objectAtIndex:indexPath.row]];
    }
}
