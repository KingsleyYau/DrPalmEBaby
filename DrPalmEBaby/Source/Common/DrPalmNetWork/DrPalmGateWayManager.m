...
#define CENTERURL @"http://%@/netapp/getAppGw?schoolkey=%@&numno=%@&dpi=%@&dspwidth=%@&dspheight=%@&os=%@&modelno=%@&appver=%@&tokenid=%@"
...
#define NUMNO       @"numno"
#define DPI         @"dpi"
#define DSPWIDTH    @"dspwidth"
#define DSPHEIGHT   @"dspheight"
#define OS          @"os"
#define MODELNO     @"modelno"
...
{
    return [self getGateWays:delegate schoolKey:_schoolKey];
}
...
-(BOOL)getGateWays:(id<DrPalmGateWayManagerDelegate>)delegate schoolKey:(NSString*)schoolKey
{
    ...
    if (!_handling){
        ...
        NSString *deviceDPI = [NSString stringWithFormat:@"%.0f", [UIScreen deviceDPI]];
        NSString *dspwidth = [NSString stringWithFormat:@"%.0f", [UIScreen mainScreen].bounds.size.width];
        NSString *dspheight = [NSString stringWithFormat:@"%.0f", [UIScreen mainScreen].bounds.size.height];
        
        NSDictionary *infoDict = [[NSBundle mainBundle] infoDictionary];
        NSString *version = [infoDict objectForKey:@"CFBundleVersion"];
        
        NSString *os = [[[UIDevice currentDevice] systemName] UrlEncode];
        NSString *model = [[NSString stringWithFormat:@"%@ %@", [[UIDevice currentDevice] model], [[UIDevice currentDevice] systemVersion]] UrlEncode];
        ...
        NSString *urlString = [NSString stringWithFormat:CENTERURL, AppEnviromentInstance().clientEntitlement.centerDomain, schoolKey, @"10", deviceDPI, dspwidth, dspheight, os, model, version, deviceToken];
        NSURL* centerUrl = [NSURL URLWithString:urlString];
        NSLog(@"getGateWays:%@", centerUrl);
